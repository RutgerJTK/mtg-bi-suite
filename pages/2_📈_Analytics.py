...
# ══════════════════════════════════════════════════════════════════════════════
# Section 6 — Article Intelligence
# ══════════════════════════════════════════════════════════════════════════════
st.markdown('<div class="section-header">🎴 Article Intelligence</div>', unsafe_allow_html=True)
...
with col_a1:
    if 'name' in articles_df.columns:
        top_cards = (
            articles_df['name'].value_counts()
            .head(10).reset_index()
        )
        top_cards.columns = ['Card', 'Count']
        top_cards = top_cards.sort_values('Count')

        fig_top = px.bar(
            top_cards,
            x='Count', y='Card',
            orientation='h',
            labels={'Count': 'Copies Sold', 'Card': ''},
            color='Count',
            color_continuous_scale=GRAD,
        )
        fig_top.update_traces(hovertemplate='<b>%{y}</b><br>%{x} copies<extra></extra>')
        fig_top.update_layout(
            **PLOTLY_BASE,
            title=dict(text='Top 10 Most Sold Cards', font=dict(color=MUTED, size=12), x=0),
            coloraxis_showscale=False,
...
with col_a2:
    if 'name' in articles_df.columns:
        top_rev_cards = (
            articles_df.groupby('name')['card_prices']
            .sum().round(2).sort_values(ascending=False)
            .head(10).reset_index()
        )
        top_rev_cards.columns = ['Card', 'Revenue']
        top_rev_cards = top_rev_cards.sort_values('Revenue')

        fig_rev_cards = px.bar(
            top_rev_cards,
            x='Revenue', y='Card',
            orientation='h',
            labels={'Revenue': 'Total Revenue (€)', 'Card': ''},
            color='Revenue',
            color_continuous_scale=GRAD,
        )
        fig_rev_cards.update_traces(
            hovertemplate='<b>%{y}</b><br>€%{x:,.2f}<extra></extra>',
        )
        fig_rev_cards.update_layout(
            **PLOTLY_BASE,
            title=dict(text='Top 10 Cards by Total Revenue', font=dict(color=MUTED, size=12), x=0),
            coloraxis_showscale=False,
        ...
        st.plotly_chart(fig_rev_cards, use_container_width=True)
